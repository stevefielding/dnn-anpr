...
/*
    Based on examples/dnn_mmod_train_find_cars_ex.cpp
    Very slight mods, to load different annotations files, and alter window size
...
int main(int argc, char** argv) try
{
    ...
    load_image_dataset(images_train, boxes_train, data_directory+"/plateAnnotations_train.xml");
    load_image_dataset(images_test,  boxes_test,  data_directory+"/plateAnnotations_test.xml");
    ...
    trainer.set_test_iterations_without_progress_threshold(25);
    ...
    while(trainer.get_learning_rate() > 0.1)
    {
        ...
        if (cnt%30 != 0 || images_test.size() == 0)
        {
            ...
            // images and boxes.
#ifdef CHECK_CROPPED_IMAGES
            for (size_t i = 0; i < mini_batch_samples.size(); ++i)
            {
                win.clear_overlay();
                win.set_image(mini_batch_samples[i]);
                for (auto b : mini_batch_labels[i])
                {
    ...
    net.clean();
    serialize("mmod_lplate_detector.dat") << net;
